...
// prototypes
void recherche_recur(t_graph_mat *g, t_vertex x, t_vertex y, char ***sommets, t_type *type);
t_bool recherche_recur_f(t_graph_mat *g, t_vertex x, t_vertex y, t_bool *marking, t_stack *stack);
...
t_bool recherche_recur_f(t_graph_mat *g, t_vertex x, t_vertex y, t_bool *marking, t_stack *pstack)
{
  ...
  else
  {
    if (marking[x] == 0)
    {
      ...
      // parcours de la matrice d'adjacence pour trouver les successeurs de x :

      for (w = 0; w < g->size; w++)
      { // parcours de tous les sommets
        if (g->m[x][w] == 1)
        { // si w est un successeur de x
